"""LoRA state extraction and FedAvg aggregation (no unaccounted DP noise)."""
...
def is_trainable_key(key: str) -> bool:
    ...
    return (
        "lora_" in lowered
        or lowered.endswith("classifier.weight")
        or lowered.endswith("classifier.bias")
        or ".score." in lowered
    )
...
def clip_delta(
    ...
    """Communication-bound clipping only — not a differential privacy mechanism."""
    if clip_norm <= 0:
        return delta
