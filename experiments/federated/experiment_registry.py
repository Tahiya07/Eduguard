...
def build_registry(repo_root: str, py: str) -> List[ExperimentSpec]:
    ...
    return [
        ...
        ExperimentSpec(
            ...
            phase=8,
            description="C9: Federated DP on FedProx IID (blocked until C8 passes)",
            resource_class="GPU_REQUIRED",
            priority="core",
            command=[
                py,
                ...
                "fedprox_iid_dp_r20",
                "--output",
                f"{r}/artifacts/federated/results/federated_dp_fedprox_iid_r20.json",
                "--global-adapter",
                f"{r}/artifacts/federated/models/qwen_bloom_federated0.5B_fedprox_iid_dp_r20",
                "--noise-multiplier",
                "0.5",
                "--target-delta",
                "1e-5",
            ...
            expected_outputs=[f"{r}/artifacts/federated/results/federated_dp_fedprox_iid_r20.json"],
            gate="dp_validated",
            blocking=False,
            config_path=f"{r}/experiments/federated/configs/federated_dp_fedprox_iid.json",
        ),
        # --- Phase 9: SecAgg ---
        ExperimentSpec(
