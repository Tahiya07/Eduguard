...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--dataset-dir",default="data/bloom_rewrite_versions/bloom_rewrite_synth_v4_1"); ap.add_argument("--split",choices=["train","validation","all"],default="all"); ap.add_argument("--min-semantic",type=float,default=.62); args=ap.parse_args()
    ...
    for r in rows:
        ...
        v=validate_candidate(r["source_question"],r["target_bloom_level"],r["target_rewrite"],semantic_similarity=semantic,min_semantic_similarity=args.min_semantic)
    ...
    report={"n":len(rows),"failures_on_revalidation":dict(failures),"exact_source_copies":exact,"known_v3_template_phrases":generic,"transformation_counts":dict(trans),"semantic_model":"BAAI/bge-small-en-v1.5","min_semantic":args.min_semantic,"clean":not failures and not exact and not generic}
