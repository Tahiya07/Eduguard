...
for i, line in enumerate(lines):
    if 'def _is_trivial_transformation' in line:
        func_start = i
    if func_start is not None and i > func_start and line.startswith('def ') and i > func_start + 10:
        func_end = i
        break

if func_start and func_end:
    # Write new simplified function
    new_func = '''def _is_trivial_transformation(original: str, rewrite: str, target_level: str) -> tuple[bool, str]:
    """
...
'''
    ...
    print(f'Replaced function at lines {func_start} to {func_end}')
else:
